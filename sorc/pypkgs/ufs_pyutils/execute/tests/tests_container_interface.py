...
class TestContainerMethods(TestCase):
    ...
    def setUp(self: TestCase):
        ...
        yaml_file = os.path.join(
            self.dirpath, "tests", "test_files", "test.hello_world.yaml"
        )
    ...
    def test_build_sfd_local(self: TestCase):
        ...
        (stderr, stdout) = [
            os.path.join(self.dirpath, "tests", "hello_world_sfd.err"),
            os.path.join(self.dirpath, "tests", "hello_world_sfd.out"),
        ]

        sif_name = container_interface.build_sfd_local(
            build_dict=build_dict, stderr=stderr, stdout=stdout
        )
        ...
        self.assertTrue(exist, msg=(self.unit_test_msg.format("build_sfd_local")))
    ...
    def test_cleanup(self: TestCase):
        ...
        for yaml_key, yaml_value in self.yaml_dict.items():
            sif_name = parser_interface.dict_key_value(
                dict_in=self.yaml_dict[yaml_key],
                key="sif_name",
                force=True,
                no_split=True,
            )
