...
---------
    ...
    __database_execute__(cursor, exec_str, is_read=False)

        This function executes a SQLite3 library API statement.

    __database_exist__(path)
...
def __database_close__(connect: object) -> None:
    """
    Description
    -----------
...
def __database_commit__(connect: object) -> None:
    """
    Description
    -----------
...
def __database_connect__(path: str) -> Tuple[object, object]:
    """
    Description
    -----------
...
def __database_execute__(
    ...
    table: ``Dict``
        ...
        database table; this is returned only if `is_read` is `True`
        upon entry; otherwise NoneType is returned.
...
def __database_exist__(path: str) -> bool:
    """
    Description
    -----------
    ...
    # Check whether the SQLite3 database path exists.
...
def create_table(path: str, table_name: str, table_dict: Dict) -> None:
    ...
    (connect, cursor) = __database_connect__(path=path)
    try:
        ...
        try:
            ...
            try:
                ...
                # Execute the SQLite3 database table command.
                __database_execute__(cursor=cursor, exec_str=exec_str)
            ...
            __database_commit__(connect=connect)
    ...
    __database_close__(connect=connect)
...
def read_table(path: str, table_name: str) -> Dict:
    ...
    # accordingly.
    exist = __database_exist__(path=path)
    if not exist:
        msg = (
    ...
    try:
        ...
        (connect, cursor) = __database_connect__(path=path)
        exec_str = f"SELECT * FROM {table_name}"
        table = __database_execute__(cursor=cursor, exec_str=exec_str, is_read=True)
...
def read_tablenames(path: str, format_list: bool = False) -> List:
    ...
    # Define the SQLite3 database path connection.
    (_, cursor) = __database_connect__(path=path)
